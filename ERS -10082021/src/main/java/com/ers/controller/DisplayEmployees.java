	...
	/*	HttpSession session=request.getSession(true);
		...
		*/
		...
		for(EmployeeEntity e:elist) {
			...
			out.println("<tr><td>"+e.getEmployeeName()+"</td><td>"+e.getEmail()+"<td>");
			Set<ReimbursementRequest> s1=e.getRequestset();
			for( ReimbursementRequest r : s1) {
			out.println(r.getRid()+"</td><td>  "+r.getStatus()+" </td><td>  "+r.getAmount()+" </td><td> "+r.getStatus()+"</td></tr>");
			}
		}
